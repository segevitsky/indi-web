...
CREATE OR REPLACE FUNCTION _fake_session_events(steps text[], methods text[], base_durs int[])
RETURNS jsonb AS $$
DECLARE
  events jsonb := '[]'::jsonb;
  t bigint := 0;
...
BEGIN
  FOR i IN 1..array_length(steps, 1) LOOP
    ...
    events := events || jsonb_build_object(
      'step', steps[i], 'method', methods[i], 'status', 200, 'tOffsetMs', t, 'durMs', dur
    );
...
BEGIN
  FOR day_offset IN 0..89 LOOP
    ...
    -- Employee day-to-day sessions (a few realistic variants)
    ...
    -- Manager sessions — clicking through several direct reports (the universal repeat pattern)
    ...
    -- HR admin sessions — report-status polling (the HR-only planted waste)
    FOR i IN 1..n_hr LOOP
      ...
      events := _fake_session_events(poll_steps, poll_methods, poll_durs);
      total_dur := (SELECT MAX((e->>'tOffsetMs')::bigint + (e->>'durMs')::bigint) FROM jsonb_array_elements(events) e);
